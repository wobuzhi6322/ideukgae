...
**"흰 배경 위의 위계"** — 색으로 구분하지 않고 여백·굵기·그레이스케일로 구분한다.
액센트는 로고에서 온 블루(#0F6BFF) 단 하나. 상태색(green/danger/yellow)은 뱃지·알림에만.
그라데이션, 블롭, 장식용 배경 금지(v1 Don'ts 승계). 다크 모드 없음(단일 테마) — theme-toggle은 CSS로 전역 숨김.

## 토큰 (변수명은 구버전 호환 — 값만 v2)
